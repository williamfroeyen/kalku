...
function updateResult() {
    ...
    if (!isNaN(input1Value) && !isNaN(input2Value)) {
        ...
        outputValue = Math.round((outputValue + Number.EPSILON)*100000)/100000;
        outputValueLength = (outputValue + '').replace('.', '').length;

        if (outputValueLength > 12) {
            outputTextElement.textContent="For mange tall";
        } else {
            outputTextElement.textContent=`Frekvens: ${outputValue} s`;
        }
    } else {
        outputTextElement.textContent="Frekvens: ";
    }
};